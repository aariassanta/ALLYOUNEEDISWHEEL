...
def check_and_install_dependencies():
    ...
    for req in requirements:
        # Extract package name (handle different formats)
        package_name = req.split('>=')[0].split('==')[0].split('>')[0].split('<')[0].split('<=')[0].strip()

        # Handle GitHub installs with #egg=
        if '#' in req and '=' in req:
            egg_part = req.split('#', 1)[1]
            if egg_part.startswith('egg='):
                package_name = egg_part.split('=', 1)[1]

        # Handle @ syntax for pip install name @ url
        if '@' in package_name:
